...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 3) {
            ...
            System.err.println("Usage: icfpc.Main input.txt a b maxTime");
            System.exit(-1);
        }
        String filePath = args[0];
        int A = Integer.valueOf(args[1]);
        int B = Integer.valueOf(args[2]);
        int maxTime = args.length > 3? Integer.valueOf(args[3]): 100;
    ...
    public static void selfTest() {
        ...
        Board b = Board.parse(factorial);
        b.setA(10);

        int i = 0;
        while(b.history.getResult() == null && i <= 100) {
            b = b.next();
            i++;
        }

        int result = b.history.getResult().asInt().orElseThrow().value().intValue();
        if (result != 3628800) {
            throw new IllegalStateException("SELF CHECK FAILED!");
        }
    }
}
